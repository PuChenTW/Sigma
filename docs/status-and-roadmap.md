...
- Playwright E2E coverage for the primary approve and reject paths.
...
- Research output uses deterministic SMR fixtures.
- There is no live source discovery, market data, or LLM dependency in the demo path.
...
- Add URL/RSS/transcript ingestion into project evidence.
- Add manual note ingestion.
- Add optional schema-validated LLM artifact generation.
- Keep deterministic fixtures as test fallback.
- Add validation that generated artifacts and theses cannot cite missing evidence.
...
This keeps the current vertical slice intact while replacing the least realistic part of the MVP: curated evidence and deterministic research text.
